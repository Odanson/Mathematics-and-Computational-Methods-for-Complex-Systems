...
# ╔═╡ 82dbee20-87e9-4ce2-b3ef-016fcf0da80e
answer_box(md"""
I could write it out...but honestly, [this](https://www.khanacademy.org/math/ap-calculus-ab/ab-differentiation-2-new/ab-diff-2-optional/v/quotient-rule-from-product-rule) is better.
""") 
...
- Enrico Caprioglio (TA) made some very nice code for visualising differentiation. It's an extra file you can download from the week 6 notebook webpage
"""
...
function plot_f_and_deriv(f, Δx)
	...
	plot!(p, true_derivative_of_f, linewidth=4, label="actual derivative")
end
...
# ╔═╡ 94197945-8c09-49df-a317-26c248d85586
answer_box(md"""
They correspond pretty well! If we extend the $x$-axis this will no longer be true. The frequency of the cos wave gets faster and faster, and thus the derivative starts to change sign (positive to negative to positive) over really small intervals that are smaller than $\Delta x$
""")
...
# ╔═╡ 76c81f9a-aa13-416f-92a6-3b79a024291e
J(mf)(x)*v

# ╔═╡ 61e959ee-9056-4077-bd4b-8489e5b9d87c
answer_box(md"""
1. In code: `mf(x + w) ≈ mf(x) + J(mf)(w)` if `norm(w,2)` (norm requires LinearAlgebra package) is small

(or in maths: $$f(x + w) \approx f(x) + \Big(\frac{\d f}{\d x}(x)\Big)w$$ if $\| w\|_2$ is small

So we want a $w$ such that `J(mf)(w) ≈ -0.001[1,1,1]`, i.e. all components of `mf(x)` are decreased about equally, and by a small amount) (I chose $0.001$ as an arbitrary coefficient to make it a 'small' amount)

Now `J(mf)(x)` is a matrix, so actually we are solving a matrix equation:

`J(mf)(x)(w) = -0.001[1,1,1]`

J(mf)(x) is nonsingular, so this will have an exact solution. We just need to find it:
""")

# ╔═╡ a61f4955-88e6-427e-8d60-8ee40e372f7b
w = -0.0001*(J(mf)(x) \ [1;1;1])

# ╔═╡ 96f06ce9-f24c-420d-9961-ef2993db56e0
mf(x+w) - mf(x)
...
# ╔═╡ c027859d-39e2-4d98-b9df-32957dc46c17
answer_box(md"""
The answer is in the link and the video
""")
