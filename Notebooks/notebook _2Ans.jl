...
# ╔═╡ 5e62d40b-3a63-430d-ad76-9f095132aee9
cardinality(S)
...
# ╔═╡ 5ddce1cb-a7c5-4a0e-a729-6b7890cbc56f
test_set = Set((1, "hi", 3.0))

# ╔═╡ fbafccee-07e3-423c-83f6-c0ddb60f878a
get_strings(s::Set) = (si for si in s if si isa String)

# ╔═╡ eb0e4d49-463d-4146-a23c-6f15fc180d2a
collect(get_strings(test_set))

# ╔═╡ 6328f021-9ed6-4335-b6f0-a43c5b81ad60
str2symbol(itr) = (Symbol(it) for it in itr)

# ╔═╡ 181ae32a-b6cf-42e0-b67b-6a5aefae165d
Set((str2symbol ∘ get_strings)(test_set))

# ╔═╡ 2401b66c-3d49-4d09-8d0f-5dae5d9e3b6a
...
# ╔═╡ c9692bef-1075-4b5d-a039-398fc7328c4c
begin
	i64_array = Array{Int64}(collect(1:1000))
	eltype(i64_array)
end

# ╔═╡ 961b05bb-8710-446a-8253-cb1ecd84f82b
begin
	any_array = Array{Any}(collect(1:1000))
	eltype(any_array)
end

# ╔═╡ 10e51f67-213e-4782-8976-eb44dba2216a
@time sum(i64_array)

# ╔═╡ 2725cb94-855b-484e-810a-e4a24587e1aa
@time sum(any_array)

# ╔═╡ 5be2c9b3-6c58-4867-8bcb-ea76b4bbb2ac


# ╔═╡ db73f81c-5011-4d9a-ac40-7d76fcb6f87a
function in_place_doubling!(a::Array)
	
	double(s::String) = "double"*s
	double(n::Number) = 2n
...
# ╔═╡ 56c5c059-049b-4a2e-af6c-a4d4895172ea
begin
	test_arr = collect(1:10)
	in_place_doubling!(test_arr)
	println(test_arr)
end

# ╔═╡ 5a678e22-999b-4245-981f-874f50df0ac9
begin
	test_str_arr = Array(["Hello", "World", "!"])
	println(test_str_arr)
	in_place_doubling!(test_str_arr)
	println(test_str_arr)
end
...
# ╔═╡ bc70d4d1-5789-402e-afcf-82644167506a
B1 = [i for i in 1:100 if i % 3 == 0]

# ╔═╡ bb9a75bf-d847-4360-8fe1-b73d873e8c2f
...
answer_box(md"""
	- The type of a tuple contains type information on each individual element in the tuple. (See `(1, "hi")` below). 
	- The type of an array is the most specific abstract type that all elements of the array belong to. The *least common denominator* of the type. For `[1, "hi"]` below, this is the type `Any`, as there is no more specific abstract type that encompaasses both numbers and strings.
""")
...
#Answer: packed_inputs is a single tuple, but f has no method defined
#for single inputs. it requires two inputs.
...
!!! info "Question"
	...
	$$3\sum_{i=1}^{33} B[i] = \sum_{i=1}^{100} i$$
...
# ╔═╡ a6476902-6a33-454e-8070-d20d01d52d9a
3sum(B1)

# ╔═╡ a4fa7d17-9b74-4f41-8e1a-ccdcdae304d8
sum(1:100)
...
!!! info "Answer"
	1. See above.
	2. $\bigg[\frac{n(n+1)}{2} + 1\bigg]_{i=0}^{99}$
	3. See below.
"""

# ╔═╡ df23476f-1ac3-40a0-8bcf-33cddaa23fc4
#option 1
sumit1 = (n*(n+1)/2 + 1 for n in 0:100) 

# ╔═╡ b10429fd-60bf-4375-a2e3-92cb4ee76b96
collect(sumit1)

# ╔═╡ ad569611-f68f-48ef-880a-8ef2ca182bcc
# option 2
sumit2(i) = (sum(1:j) + 1 for j in 0:i)

# ╔═╡ 6df3788a-13de-41ac-88e9-1cecbc27531e
sumit2(4) |> collect
...
# ╔═╡ 50db1d98-15d7-43ba-beea-95e64a9b83b9
fizzbuzz_without_ifs(1:20) |> collect

# ╔═╡ 281f3525-5e23-4b40-9dc0-2b4ee69a0627
...
# ╔═╡ cd38fad0-8d77-4a4d-8e70-a998eee6a621
my_sum(A)

# ╔═╡ a8f8b4fd-1e28-46e0-9fe9-156f1d3fe15c
#option 1
...
# ╔═╡ f02a6bd7-b01e-480a-8db5-23c75c260c3d
row_and_col_mean1(A)

# ╔═╡ f962202e-015d-446d-a911-a080f59a3e79
row_and_col_mean2(A)

# ╔═╡ 368bfc2c-644a-46c9-84a4-b5261e68d5bd
layer_sum(x) = sum(x; dims=(3,))

# ╔═╡ 18f75bce-837e-4d5d-b4c0-90eb42d1bfb0
begin
...
# ╔═╡ b4bc6fa9-88ca-436c-a94a-24f5215f9d28
...
# ╔═╡ 2b5ca238-3c78-4553-8c49-27b488a1d9c8
...
# ╔═╡ f9c79e60-6424-4aeb-a9c7-eb577460a3c5
recursive_bubble_sort([10.,15.,1.,-2., -2., 64., 15.0, 3.14])
...
# ╔═╡ d259a709-c3cc-4fa8-a31b-dfc5442301eb
...
# ╔═╡ 1d1b448e-6504-43c9-9a1f-810e559c5687
...
# ╔═╡ 57afd456-d0a0-4b03-b064-e6249f919add
begin
