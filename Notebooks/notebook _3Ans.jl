...
tensor_4d = [1;2;1;2;;;;3;4;3;4;;;;5;6;5;6;;;;7;8;7;8]
...
img = load(download("https://spaces.filmstories.co.uk/uploads/2020/07/the-matrix-dvd.jpg"));

# ╔═╡ 3c74cf3e-94fd-4efa-b49d-43820e0468e5
typeof(img) 

# ╔═╡ d7ed2aed-d75a-4e7a-aa88-a365189e385b
eltype(img) # can look up RGB in the live docs.
...
# ╔═╡ c24ef3ca-8148-41c7-8b82-186cd0c6c3d2
invert_pixel(x::RGB) = RGB(1 - x.r, 1 - x.g, 1 - x.b)

# ╔═╡ 122f01f9-5e98-47cf-b306-a9b072b2280a
...
# ╔═╡ 5e7a1450-2693-4eab-9690-85bfd448673d
...
# ╔═╡ c91f4b2f-d53d-4110-b23a-3bf0fb36f9d9
cheating_flip_im(img, tb=false, lr=true)

# ╔═╡ a0bf1d12-5e45-4120-959c-b7c635a14ae4
[1,2,3,4]
...
begin
	img2 = mktemp() do fn,f
	    download("https://facts.net/wp-content/uploads/2023/09/22-facts-about-enzo-matrix-reboot-1693733388.jpg", fn)
	    load(fn)
	end
	img2 = imresize(img2, (img |> size))
end
...
# ╔═╡ 3bb1dc24-2afa-4a5c-aaae-e24705ed1d4d
answer_box(md"""
See below. There are many ways to solve this problem. Note that ```numpy.ndarray.fill``` is the equivalent function in python. 
""")

# ╔═╡ f930ba2c-d010-4b72-8247-1e36de459747
red(direction::String) = fill(RGBA(1.,0.,0.,1.), direction=="vertical" ? (12, 5) : (4, 15)) # (12, 5) and (4,15) are the sizes of the filled arrays

# ╔═╡ d13bde28-93bc-4878-a9a2-becb8400c295
white(direction::String) = fill(RGBA(1.,1.,1.,1.), direction=="vertical" ? (12, 5) : (4, 15))

# ╔═╡ 9a9552f8-fc8a-4585-a98f-0e8b03792999
blue(direction::String) = fill(RGBA(0.,0.,1.,1.), direction=="vertical" ? (12, 5) : (4, 15))

# ╔═╡ a00b11e9-285b-4c87-bc9f-9666876d30fa
french_flag=hcat(blue("vertical"), white("vertical"), red("vertical"))

# ╔═╡ cb50f1f8-2644-49bf-90aa-b48ec6b53a85
dutch_flag=vcat(red("horizontal"), white("horizontal"), blue("horizontal"))
...
md"""
!!! info "Answer"
	- Not all elements in $\mathbb{N}$ have additive or multiplcative inverses, so it is not a number system.
...
another_tensor_4d = [a + b + c*d for a in 1:3, b in 1:2, c in 1:4, d in 1:3]
...
# ╔═╡ 7a23c757-4aa6-43ce-9633-86ddcea3869b
long_flip_im(img; tb=false, lr=true)

# ╔═╡ 6c1f3788-c048-4e33-984c-b3b91a4201f7
function crop_right_half_im(x::Matrix{T}) where T <: RGB
	m, n = size(x)
...
Here's a different type of grouptin:
...
# ╔═╡ 4a366a8e-57f9-44e7-b96c-d09807eac082
img .* img2

# ╔═╡ c6f79557-c158-4fcc-8fb3-07f9441a5326
3*img
...
# ╔═╡ f90c195b-4fca-45e6-8cd3-8b7f559fc739
...
# ╔═╡ cc4c116c-a7be-4680-98b0-eaa4353474c7
md"""
!!! info "Answers"
	1. It belongs to the set $\mathbb{R}^4$.
	2. It belongs to the set $\mathbb{R}^{1 \times 4}$.
	3. See below.  They cannot be added together because the dimensions do not match.
"""

# ╔═╡ cb08d31c-822f-438d-87f7-dd39d9f5dead
begin
	v3216 = [3,2,1,6]
	m3216 = [3;;2;;1;;6]
	println("Type v: ", typeof(v3216), ", type m: ", typeof(m3216))
end

# ╔═╡ d9b6a031-33b5-4d9a-8ff3-3b501644e339
v3216 + m3216
...
# ╔═╡ 8309671e-c8bc-457d-a121-4a476f0a5af9
md"""
!!! info "Answers"
	Yes, $\mathbb{R}^{m\times n}$ is a vector space.  The zero element is an $m\times n$ matrix filled with 0's.
"""
...
4. The slider traverses only a small subset of the space of possible $382 \times 600$ images consisting of RGB pixels. How many sliders would you need to traverse all such images? What would be a better choice of basis set for these sliders?
...
# ╔═╡ cdf1e6d5-b54a-45ed-8d6f-50f66e393e83
md"""
!!! info "Answers"
	1. The set can be described as: $S = \{\alpha v_1 + (1 - \alpha) v_2 \ | \ 0 \leq \alpha \leq 1 \}$.  This is a line in the space of $382 \times 680$ images, where the endpoints are the two images $v_1$ and $v_2$.
	2. It would be an affine space.  It cannot be a vector space because there is no zero vector - that is to say, there is no origin.  
	3. This is a 1 dimensional space, for it is spanned by a single vector: $v_2 - v_1$.
	4. You would require $382 \times 680 \times 3 = 687600$ sliders.  A better basis would be a collection of images, where each image is black (0,0,0) for all pixels, except at a single point, where it is either red (1, 0, 0), green (0,1,0), or blue (0,0,1).
    5. This space has 687600 dimensions.
	6. The image file would be about $(382 \times 680) \text{ pixels} \times 1 \text{ byte/pixel} = 259.76 \text{ kilobytes}$.
"""
...
# ╔═╡ 52f63b74-f2ec-482a-af4d-83397f4420ce
g(v) = B*v

# ╔═╡ 26ed7200-c1e4-4e89-9acd-76dd0900946f
h(v) = A*B*v

# ╔═╡ a4f950b1-683f-4359-8f99-fd689ccb8b51
(f ∘ g)([2, 1]) == h([2, 1])
