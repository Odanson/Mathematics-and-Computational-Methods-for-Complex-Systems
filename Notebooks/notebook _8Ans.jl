...
# ╔═╡ 1be13133-e6b9-40ba-bc66-af708dab48d9
answer_box(
	md"""
	Using $x(t) = x_1(t)$, $\dot x(t) = x_2(t)$, and $\ddot x(t) = x_3(t)$ we have the system of first order ODEs:
	```math
	\begin{align}
	&\dot x_1(t) = x_2(t) \\
	& \dot x_2(t) = x_3(t) \\
	& \dot x_3(t) = x_3(t) + 2x_2(t) - x_{1}(t)^2
	\end{align}
	```

	The first two equations come directly from our definitions of $x_1(t) = x(t)$, $x_2(t) = \dot x(t)$, and $x_3(t) = \ddot x(t)$, while the third equation is the original ODE but with the newly defined variables plugged in.
	
	Note, we started with one **third-order** differential equation and now we have three **first-order** differential equations, no information has been lost. On the LHS we have first order derivatives and on the RHS we do not have derivatives anymore.
	"""
)
...
# ╔═╡ e7264be1-8043-44ec-85d9-5c3a5f4d91d0
md"""
Click on the cell below to see how to write the second order DE as a matrix equaiton of two ODEs:
"""

# ╔═╡ d3fd48e8-ca68-4dbd-9fd4-1b7bc7f3c20c
answer_box(
md"""
Given
...
note that we can write $x_1(t) = x(t)$ and $x_2(t) = \dot{x}(t) = \dot{x}_1(t)$. Then

$$\begin{cases}
      \dot{x}_1(t) = x_2\\
      \dot{x_2}(t) = \frac{1}{m}\left(-c x_2(t) - kx_1(t) \right)
    \end{cases}$$

Where the first equation was obtained from our definitions of $x_1(t)$ and $x_2(t)$, while the second equation from the original damped oscillator equation.

To write it as a matrix equation, first notice that on the LHS we only have $\dot{x}_1(t)$ and $\dot{x}_2(t)$, while on the RHS we have some expressions in terms of $x_1$ and $x_2$ multiplied by some coefficients. With some intuition, it is already possible to write the matrix equaiton given this system of equations. But we can also work it out more explicitly by considering the following matrix expression:
...
We then perform the matrix multiplication on the RHS.
...
The result is the same as the following system of equations:

$$\begin{cases}
\dot{x}_1(t) = px_1(t) + qx_2(t)\\
\dot{x_2}(t) = rx_1(t) + sx_2(t)
\end{cases}$$

By comparing this last expression with the previous system of equations, we can see that $p=0$, $q=1$, $r=-\frac{k}{m}$, and $r=-\frac{c}{m}$.
"""
)
...
# ╔═╡ 0d2e821f-b80e-453a-b32f-7bbe5b710452
md"""
**Answer Question 1**
"""

# ╔═╡ f768567e-9728-44db-b980-ae94674abf0c
answer_box(
md"""	
Recall the chain rule: given $x = f(u)$, $u = g(t)$ so that $x = f(g(t))$ then

$$\frac{\mathrm{d}x}{\mathrm{d}t} = \frac{\mathrm{d}x}{\mathrm{d}u}\frac{\mathrm{d}u}{\mathrm{d}t}.$$

Hence we can rewrite $x(t) = A \sin \left(\sqrt{\frac{k}{m}}t + \phi \right)$ as
```math
\begin{align}
x(t) &= A\sin\left( u(t) \right), \\
u(t) &= \sqrt{\frac{k}{m}}t + \phi\quad \text{and} \\
a &= \sqrt{\frac{k}{m}},
\end{align}
```
where the new variable $a$ is used to make the calculations less messy. Note that $a$ only depends on $k$ and $m$. Since we are not differentiating or integrating with respect to these variables it's just easier to treat the factor $\sqrt{\frac{k}{m}}$ as a single "less scary" variable $a$.

Firstly, evaluate $\frac{\mathrm{d}x}{\mathrm{d}u}$:

$$\frac{\mathrm{d}x}{\mathrm{d}u} = \frac{\mathrm{d}}{\mathrm{d}u}\left(A\sin(u)\right) = A\cos(u)$$

and secondly, evaluate $\frac{\mathrm{d}u}{\mathrm{d}t}$:

$$\frac{\mathrm{d}u}{\mathrm{d}t} = \frac{\mathrm{d}}{\mathrm{d}t}\left(at + \phi\right) = a.$$
...
\frac{\mathrm{d}x}{\mathrm{d}t} &= \frac{\mathrm{d}x}{\mathrm{d}u}\frac{\mathrm{d}u}{\mathrm{d}t} \\
&= A\cos(u) \times a \\
&= A\cos\left(at + \phi\right) \times a
\end{align}
```

Then, the second derivative $\ddot{x}$ is:

```math
\begin{align}
\ddot{x} &= \frac{\mathrm{d}\dot{x}}{\mathrm{d}t} \\

&= \frac{\mathrm{d}}{\mathrm{d}t}\left(A a\cos\left(at + \phi\right)\right) \\

&= Aa \frac{\mathrm{d}}{\mathrm{d}u} \left(\cos(u(t))\right) \times \frac{\mathrm{d}u}{\mathrm{d}t} \\

&= - \left( a \right)^{2} A \sin\left( at + \phi \right)

\end{align}
```
where we first used again the chain rule and then subbed back in $u$. Note also that $a^2 = \left(\sqrt{\frac{k}{m}}\right)^2 = \frac{k}{m}$.

Now, we can finally substitute all these terms into the undamped oscillator equation:
```math
\begin{align}
\ddot{x}(t) &= - \frac{k}{m}x(t) \\

- \left( \sqrt{\frac{k}{m}} \right)^{2} A \sin\left( \phi + t \sqrt{\frac{k}{m}} \right) &= -\frac{k}{m} A \sin \left(\sqrt{\frac{k}{m}}t + \phi \right) \\

\frac{k}{m} A \sin\left( \phi + t \sqrt{\frac{k}{m}} \right) &= \frac{k}{m} A \sin \left(\sqrt{\frac{k}{m}}t + \phi \right)
\end{align}
```

where from the second to the third lines of the last computations we have multiplied by $-1$ on both sides and simplified the term $\left(\sqrt(k/m)\right)^2 = k/m$.

In the last expression, the LHS is equal to the RHS, which shows that indeed $x(t) = A \sin \left(\sqrt{\frac{k}{m}}t + \phi \right)$ is a solution of the undamped oscillator equation.
"""
)

# ╔═╡ ddf3f397-a6c5-4a81-a455-416a7efcf5df
md"""
**Answer Question 2**
"""

# ╔═╡ ba0392f9-9c9f-429c-8209-996961d97f66
answer_box(
md"""
Assume that we know the initial conditions $x(0)$ and $\dot{x}(0)$ (i.e., the value of $x(t)$ and $\dot{x}(t)$ at time $t=0$).

Then,

```math
\begin{align}
x(0) &= A \sin \left(\sqrt{\frac{k}{m}}\times0 + \phi \right) \\
&= A\sin(\phi) \\

\dot{x}(0) &= A\cos\left(\sqrt{\frac{k}{m}}\times 0 + \phi\right) \times \sqrt{\frac{k}{m}} \\
&= A\cos(\phi)\sqrt{\frac{k}{m}}.
\end{align}
```

We are told to assume that we have the initial conditions for $x(0)$ and $\dot{x}(0)$. Then we can let $x(0) = x_{0}$ and $\dot{x}(0) = x_1$. Similarly as before, let $a = \sqrt{\frac{k}{m}}$ as well.

Hence, we now have two equations, and two unknowns, $A$ and $\phi$.

From the expression for $x(0)$ we get an expression for $A$:
```math
A=\frac{x_0}{\sin(\phi)}.
```
However, this still depends on the other unknown $\phi$. Now, we substitute $A$ into the expression for $\dot{x}(0)$ to get

```math
\begin{align}
\dot{x}(0) &= x_{1} = A\cos(\phi)\sqrt{\frac{k}{m}}\\
&= \frac{x_0}{\sin(\phi)}\cos(\phi)a \\
&= x_0 \tan^{-1}(\phi)a \\
\Rightarrow x_{1} &= x_0 \tan^{-1}(\phi)a
\end{align}
```
We now just need to isolate $\phi$ from the previous expression:
```math
\tan(\phi) = \frac{x_0}{x_{1}} a
```
and therefore obtain
$\phi = \arctan\left(\frac{x_{0}}{x_{1}}\sqrt{\frac{k}{m}}\right)$
"""
)
...
# ╔═╡ e4bf203c-f4b2-4441-8943-44e1ef4a1775
answer_box(
	md"""
	To answer this question, try to think qualitatively. What would the results of this model translate to in the real world? In other words, does our solution, or plot make any sense?
	
	In this case, remember that we are modelling a mass attached to a spring, i.e., a classic oscillator. Take a look at the gif above at the beginning of this section. With the damping term in the equations, we are expecting the velocity of the mass to oscillate for a while. As the mass goes up (along the vertical axis which we use as our position) the velocity will also increase $> 0$. When the mass goes down (along the vertical axis) the velocity will decrease below zero.
	
	After a while, the velocity will start to oscillate above and below zero less and less as energy dissipates.
	
	Eventually, it will stop oscillating and the velocity will rest at zero. This is what happend for `tstep ≤ 0.01`.

	However, using `tstep ≥ 0.1`, the velocity doesn't decrease, it is increasing terribly instead. That would never happen in the real world.

	To see this even more clearly, try to change `tend = 10` to `tend = 100` as well!

	Note how even if the forward Euler method has been implement correctly, you may still have some huge issues if the step size is too large. In this specific example, it is not a problem to decrease the step size. However, in some cases we require a step size so small that it becomes extremely slow for a normal laptop to compute the simulation.
	"""
)
...
# ╔═╡ 0c376df3-9bf5-484a-9282-ed93fdaa3128
md"""
**Answers to question 1 and 2**
"""

# ╔═╡ be22010c-9cc0-4eab-879c-3d6f31b8b43c
answer_box(
md"""
**Dislaimer: answer may seem very long!**

But if you remove our comments, it actually consists of three steps:
- write the unknown eigenvalues as some general complex numbers: $\lambda_1 = x_1 + i x_2$ and $\lambda_2 = x_3 + i x_4$. We now have four unknowns: $x_1, x_2, x_3, x_4$ but our objective is only to show that $x_1 < 0$ and $x_3 < 0$, i.e., the real part of the eigenvalues is negative;
- use the first equation for the trace of the matrix $A$ and check the conditions for the result to be real and negative.
- then use the second equation from the matrix determinant and check again for what conditions that is true.

**Answer with more comments**

Rather than computing the eigenvalues directly, we may notice that the trace of $A$ is equal to

$$\mathrm{Tr}\left[A\right] = \sum_i A_{ii} = 0 + \left(-\frac{c}{m}\right) = -\frac{c}{m}.$$

Recall that the trace of an $n\times n$ matrix $M$ is equal to the sum of the eigenvalues of $M$. Then $\sum_i \lambda_i = \lambda_1 + \lambda_2 = -\frac{c}{m}$.

The coefficient $c$ will always be $> 0$, otherwise the damped oscillator would be the opposite of damped and have infinite energy (as it would start to oscillate more and more). Similarly, the mass will also always be greater than zero. Hence, the trace of $A$ must be less than zero (and real, since $c\in\mathbb{R}$ and $m\in\mathbb{R}$):

$$\mathrm{Tr}\left[A\right] = -\frac{c}{m} = \lambda_1 + \lambda_2 < 0.$$

Note also that, since $\mathrm{Tr}[A] \in \mathbb{R}$ then the imaginary part of the eigenvalues must cancel each other, i.e., $\mathrm{Im}[\lambda_1] = - \mathrm{Im}[\lambda_2]$.

However, we haven't shown yet that both eigenvalues' real parts are less than zero. We have only shown that their sum is less than zero and that the imaginary parts of the eigenvalues are equal in magnitude and have opposite signs. For the matrix $A$ to be stable we require both real parts of the eigenvalues to be $<0$.

We then look at the determinant of $A$:

```math
\begin{align}
\mathrm{Det}[A] &= \left[0 \times \left(-\frac{c}{m}\right)\right] - \left[1 \times \left(-\frac{k}{m}\right)\right] \\
&= 0 - \left(-\frac{k}{m}\right) \\
&= \frac{k}{m}
\end{align}
```

This is always greater than zero instead, since both $k$ and $m$ must be greater than zero. Consequently, we can also say that the product of the eigenvalues is also greater than zero, $\mathrm{Det}[A] = \lambda_1 \times \lambda_2 > 0$. We can also notice that $\mathrm{Det}[A]\in\mathbb{R}$ (since both $k$ and $m$ are real).

Now we need to consider two cases:

**Case 1: real eigenvalues**
If the eigenvalues were real, then it must be the case that $\lambda_1$ and $\lambda_2$ are either both positive or both negative. Since we know their sum must be negative, we would conclude that both $\lambda_1$ and $\lambda_2$ are negative, and that the system is stable.

**Case 2: compelx eigenvalues**
But what if the eigenvalues were complex numbers? To show that the real parts of the eigenvalues are negative write $\lambda_1 = x_1 + i x_2$ and $\lambda_2 = x_3 + i x_4$, with $x_i\in\mathbb{R}\,,\forall i = 1,2,3,4$. Then, the objective now is to show that $x_1 < 0$ and $x_3 < 0$. To do so, we can use our previous results to find the conditions for $x_1, x_2, x_3,$ and $x_4$.

**Step 1.** Using our first result: $\mathrm{Tr}[A] = \lambda_1 + \lambda_2 < 0$ and real, we find that the imaginary part of the eigenvalues must be equal in magnitude and opposite in sign, i.e.,

```math
\begin{align}
(x_1 + i x_2) + (x_3 + i x_4) &= -\frac{c}{m} \\
(x_1 + x_3) + i(x_2 + x_4) &= -\frac{c}{m} \\
\Rightarrow x_2 &= - x_4,
\end{align}
```

while $x_1 + x_3 = -\frac{c}{m} < 0$. Again, this is not quite enough, it may be the case that either $x_1 > 0$ or $x_3 > 0$. We need to use our second result as well and plug in the new expressions for $\lambda_1$ and $\lambda_2$.

**Step 2.** Using our second result instead we have that $\lambda_1 \times \lambda_2 = k/m > 0$ hence

```math
\begin{align}
(x_1 + i x_2) \times (x_3 - i x_2) &= \frac{k}{m} \\
x_1x_3 + (x_2)^2 + i (x_2x_3 - x_2x_1) &= \frac{k}{m}.
\end{align}
```
Since $\frac{k}{m}\in\mathbb{R}$ we require the imaginary part to go to zero, and the real part to be equal to $k/m$:
```math
\begin{cases}
x_1x_3 + (x_2)^2 = \frac{k}{m} \\
x_2x_3 - x_2x_1 = 0
\end{cases}
```
From the second equation we finally get that $x_3$ must be equal to $x_1$, such that the first equation becomes

$$(x_1)^2 + (x_2)^2 = \frac{k}{m},$$
note that the LHS is indeed always real and positive for any $x_1,x_2\in\mathbb{R}$.

**Finally**: recall from **step 1** we had $x_1 + x_3 = - \frac{c}{m}$ but now we know $x_1 = x_3$, therefore

$$x_1 = x_3 = - \frac{c}{2m} \quad \Rightarrow \quad \mathrm{Re}[\lambda_1] = x_1 < 0 \; \text{and} \; \mathrm{Re}[\lambda_2] = x_3 < 0.$$
"""
)

# ╔═╡ 11e30a43-6333-44ae-a32b-eeeb2e01730b
md"""
**Answers to question 3 and 4**
"""

# ╔═╡ 14f9d269-c37b-41ce-961d-b15414d03282
answer_box(
md"""
To make the system unstable, we require one of the eigenvalues of $A$ to have positive real part.

We showed earlier that if we write the eigenvalues of $A$ as complex numbers: $\lambda_1 = x_1 + i x_2$ and $\lambda_2 = x_3 + i x_4$, we obtained that

$$x_1 = x_3 = - \frac{c}{2m} \quad \text{and} \quad x_2 = - x_4$$

Such that the eigenvalues now look like $\lambda_1 = x_1 + i x_2$ and $\lambda_2 = x_1 - i x_2$

Hence, for the system to be unstable we only require $x_1 > 0$. This can be done by either selecting a negative mass, e.g., $m = -1$, or a negative damping coefficient $c = -0.1$, but not both!

To instead obtain zero real parts of the eigenvalues, i.e., $\mathrm{Re}[\lambda_1] = \mathrm{Re}[\lambda_2] = 0$ we have the option of setting $c= 0$. This is the undamped oscillator case, in which the only fixed point is a limit cycle  (i.e., an oscillation).
"""
)

# ╔═╡ 7c6c8a7f-639a-4901-809e-5b700acbd546
md"""
You can use the code below to numerically check our analytical calculations are correct
"""

# ╔═╡ f5860608-6927-4e4f-b05a-fc0f0a1770a4
let
	c = mass_spring_hyperparams["c"]
	k = mass_spring_hyperparams["k"]
	m = mass_spring_hyperparams["m"]

	# unphysical coefficients (remove # to use these commands)
	# c = -c
	# m = -m

	# real eigevalues = zero (uncomment to use commands)
	# c = 0
	...
	trace(A) = sum([A[i,i] for i in 1:size(A)[1]])
	determinant(A) = (A[1,1] * A[2,2]) - (A[1,2] * A[2,1])

	tr = trace(A)
	det = determinant(A)

	# here we use a trick to compute eigenvalues from the trace and determinant of a 2x2 matrix: see the end of the notebook for a derivation of this handy formula
	λ₁ = (1/2) * (tr + √(Complex(tr^2 - 4*det)))
	λ₂ = (1/2) * (tr - √(Complex(tr^2 - 4*det)))

	println("λ₁ = ", λ₁, "\nλ₂ = ", λ₂)
	println("\nx_1 = x_3 = -c/(2m) = $(-c/(2*m))")

	# the command below only works for physically plausible values of k,c,m
	# println("\nas a sanity check for our calculations, the imaginary part should be:\nx_2 = x_4 = k/m - (c/(2m))^2 = ", sqrt(k/m - (c/(2*m))^2))
end
...
# ╔═╡ 27c514cd-8753-4564-8017-ad53f1141ec0
answer_box(
md"""
**Question 1**

We saw earlier that the number of susceptible people is equal to the number of people minus the number of infected and recovered people, i.e., $S = N-I-R$. This is precisely what the first argument `hyps["N"] .- xs[:,1] .- xs[:,2]` in 

	`xs = hcat(hyps["N"] .- xs[:,1] .- xs[:,2], xs )`

is doing. For each day (hence the broadcast `.`), we are constructing a new column that stores the values of $S$ for each day.

**Question 2**

As the number of recovered people goes down (green line), more people can become susceptible again (blue line) due to the resusceptibility rate. But as more people become susceptible, the more people can be infected again. If the recovery rate is not strong enough, the bump disappears.
"""
)
...
# ╔═╡ 467b4a76-9bec-4e33-a3c8-fe03b809c632
md"""
Here is a possible answer:
"""

# ╔═╡ 46526082-653f-4031-9efe-550c94030aa1
function run_stochastic_simulation(hyps)
	...
	p = hyps["p"]
	γ = hyps["γ"]
	# from last week we add
	q = hyps["q"] # resusceptibility rate

	# initial conditions
	SIRL[:,1] = [S0, I0, R0]

	# define stochastic functions
	infections(i) = rand(Binomial(SIRL[1,i-1], p*SIRL[2,i-1]/N))
	recoveries(i) = rand(Binomial(SIRL[2,i-1], γ))
	resusceptibility(i) = rand(Binomial(SIRL[3,i-1], q))

	# simulation update function
	function SIRL_update(i)
		S,I,R = SIRL[:,i-1]
		inf = infections(i)
		rec = recoveries(i)
		# add the number of resusceptible people each day
		resusc = resusceptibility(i)

		# Extra: since the number of infections may go to zero we need to add some random infections every once in a while, so that it may be possible to infect new people again and obtain that bump that we saw earlier
		function rand_infection(s_i)
			if rand() < s_i
				return 1
			else
				return 0
		...
		I = I + inf - rec + rand_infection(0.1)
		R = R + rec - resusc
		# S = S - inf + resusc # or
		S = N - I - R
	...
	# run simulation
	for i in 2:days
		SIRL[:,i] = SIRL_update(i)
	end
	
	return SIRL
end

# ╔═╡ 9104c2af-c965-4297-af5f-12404cee8775
function plot_stochastic_and_ODE_SIRL(hyps)
	p = plot_SIRL(hyps)
	SIRL_stoch = run_stochastic_simulation(hyps)
	scatter!(p, SIRL_stoch', labels = ["susceptible" "infected" "recovered"])
end

# ╔═╡ 97a202f7-0292-44e1-a76b-dd56d3e0703d
begin
	Slider_a = @bind n_SIRL_students_new Slider(5:5:1000, default=300, show_value=true)

	Slider_b = @bind initial_SIRL_infections_new Slider(5:5:n_SIRL_students_new, default=1, show_value=true)

	Slider_c = @bind SIRL_days_new Slider(5:200, default=150, show_value=true) 

	Slider_d = @bind recovery_rate_new Slider(0:0.01:1, default=0.6, show_value=true)

	Slider_e = @bind infectiveness_new Slider(0.0:0.01:1, default=0.95, show_value=true)

	Slider_f = @bind resusceptibility_rate_new Slider(0.0:0.01:1, default=0.02, show_value=true)

	Slider_g = @bind R0_new Slider(0:5:n_students, default=1, show_value=true)

	Slider_h = @bind I0_new Slider(0:n_students, default=35, show_value=true)
...
# ╔═╡ 112ac447-9a12-4c67-a63c-5528733e37c3
SIRL_stochastic_hyper = Dict( 
	"N" => n_SIRL_students_new,
	"I₀" => I0_new,
	"R₀" => R0_new, # initial recovered
	"days" => SIRL_days_new,
	"p" => infectiveness_new,
	"γ" => recovery_rate_new,
	"q" => resusceptibility_rate_new,
	"δt" => euler_step,
)

# ╔═╡ 7a603635-783a-4727-a227-8e60e7418cb7
plot_stochastic_and_ODE_SIRL(SIRL_stochastic_hyper)

# ╔═╡ b5a961ca-7f71-4db4-afc0-c6c17f34d64c
md"""
Run the above cell a few times. The stochastic simulation is not always exactly the same as the ODE simulation!
"""

# ╔═╡ ff0589cd-4c12-4fb6-bb83-5afc38096f2c
md"""
**EXTRA**: *definitely not needed for the exam.*

Comment on the `function rand_infection(s_i)`.

Notice that the rate of infections is

$$\dot{I} = p\frac{SI}{N} - \gamma I$$

of course, if the number of infected people goes to zero, the rate of new infections will stay constant at zero.

In the stochastic simulation we modelled this using two Binomial distributions, one for the random variable of infected people, and one for the random variable of recovered people. If $I=0$, then we will always have zero new infections. In the stochastic model it is possible that the number of infected people goes to zero.

In contrast, in the simulation using the Euler step, the number of infected people never actually goes to zero. To see this take a look at the code below:
"""

# ╔═╡ f2d8bd07-82f2-4c81-bc0d-37e1878d7a58
let
	# run Euler simulation
	ts, SIRL_euler = forward_euler_solve(
		build_SIRL_dynamics(SIRL_stochastic_hyper), 
		build_initial_conditions(SIRL_stochastic_hyper),
		0,
		SIRL_stochastic_hyper["days"],
		SIRL_stochastic_hyper["δt"]
	)
	
	SIRL_euler = hcat(SIRL_stochastic_hyper["N"] .- SIRL_euler[:,1] .- SIRL_euler[:,2], SIRL_euler )

	println("Minimum value of I in the forward Euler simulation:\n", minimum(SIRL_euler[:, 2]))

	# run stochastic simulation
	SIRL_stochastic = run_stochastic_simulation(SIRL_stochastic_hyper)
	
	println("\nMinimum value of I in the stochastic simulation:\n", minimum(SIRL_stochastic[2, :]))
end

# ╔═╡ 44a5ebae-a5e3-420c-a206-c7bf19ca8aba
md"""
Hence, to adjust for this, we used the function `rand_infection(s_i)` to add a random number of infected people with low probability at each step:

	I = I + inf - rec + rand_infection(0.1)

If you didn't add this, it is completely fine! But you may notice that the bump in infections doesn't occur in the stochatstic model.
"""
...
# ╔═╡ 2acc1563-51d5-4f83-81aa-d51311c20665
md"""
Change $\frac{p}{\gamma}$:
$(@bind select_difference Slider(-0.3:0.01:0.39, show_value = true))
"""
...
begin
	...
	new_SIRL_hyperparameters["p"] = new_SIRL_hyperparameters["γ"] + select_difference
end

# ╔═╡ 42ece49f-ac83-46c0-9cac-04570b833f16
println("This is p/γ = $(new_SIRL_hyperparameters["p"]/new_SIRL_hyperparameters["γ"])")
...
# ╔═╡ 958ff59d-ec5d-4200-816b-c7df748e2fd4
answer_box(
	md"""
	I added a new slider to change the values of $p$ and $\gamma$ more easily. When $R_0$ is $<0$ the infections quickly go to zero and there are no oscillations.

	If $R_0 > 1$ we see that infections never truly go to zero.

	To reach the fixed point it takes longer for $R_0 > 1$, and for the maximum number of recoveries and infections.
	"""
)
...
# ╔═╡ 938f0ae1-27c2-4105-baa2-2b63bc7f6984
answer_box(
md"""
Recall the system of ODEs:
...
\end{align}
```
Then we evaluate these equations at $S^*, R^*,$ and $I^*$.

First, sub in $S^* = \frac{N \gamma}{p}$ and $R^* = \frac{\gamma}{q} I^*$ into the expression for $\dot{S}(t)$:

```math
\begin{align}
\dot{S}(t) &=  - p \left(\frac{N\gamma}{p}\right)\frac{I^*}{N}  +  q\left(\frac{\gamma}{q} I^*\right), \\
&= -\gamma I^* + \gamma I^* \\
&= 0.
\end{align}
```
Then we move on to $\dot{I}(t)$:
```math
\begin{align}
\dot{I}(t) &=  p \left(\frac{N\gamma}{p}\right)\frac{I^*}{N}  - \gamma I^*,\\
&= +\gamma I^* - \gamma I^* \\
&= 0.
\end{align}
```

And finally we check $\dot{R}(t)$ is also zero:
```math
\begin{align}
\dot{R}(t) &= \gamma I^* - q\left(\frac{\gamma}{q} I^*\right), \\
&= \gamma I^* - \gamma I^* \\
&= 0.
\end{align}
```
...
$$\begin{equation}
Tr\left(\frac{\partial f}{\partial x}\right) = \frac{ - p q - q^{2}}{q + \gamma}
\end{equation}$$

$$\det\left(\frac{\partial f}{\partial x}\right) = p q - q \gamma$$
...
# ╔═╡ 2c862788-1344-4d1b-8bfe-bbbb60640ccf
answer_box(md"""
Recall that a matrix is stable if the matrix is a [Hurwitz matrix](https://en.wikipedia.org/wiki/Hurwitz_matrix#Hurwitz_stable_matrices), i.e., it has eigenvalues with negative real part. And that a $2\times2$ matrix is Hurwitz matrix if the determinant is greater than zero (this is only for $2\times2$ matrices, check the wiki above).

We can quickly see that the condition for this to be true is 

$$\mathrm{Det}\left[\frac{\partial f}{\partial x}\right] = q(p -\gamma) > 0.$$
""")

# ╔═╡ b42c7492-31a9-48e9-b379-f96861cdac57
hint(md"""

- Weirdly, we need $q(p - \gamma)$ to be positive for the fixed point to be stable. In other words, we need the infection rate to be **higher** than the recovery rate. 
...
# ╔═╡ 4c4e4232-c914-47b7-b731-4d6f83614890
let
	# Here is some extra code if you are not convinced by this condition
	
	# change from stable = true to false to see how the eigenvalues change
	stable = true
	if stable
		q = 0.02; p = 0.9; γ = 0.6
	else
		q = 0.02; p = 0.70; γ = 0.71
	end

	# uncomment the next three lines to use the previous parameters
	# q = new_SIRL_hyperparameters["q"]
	# γ = new_SIRL_hyperparameters["γ"]
	# p = new_SIRL_hyperparameters["p"]
	
	J = [
		(-p*q+q*γ)/(q+γ) ((-p+γ)*q)/(q+γ);
		γ -q
	]
	...
	println("For q = $q, p = $p, and γ = $γ we have:\nTrace = $(tr)\nDeterminant = $(det)")

	println("\nλ₁ = $(λ₁)\nλ₂ = $(λ₂)")
end

# ╔═╡ 2d5468bf-efb0-4e1e-bfa6-439cdd94d876
md"""
# Extra by Enrico: definitely not needed for the exam
## How to calculate $2\times2$ matrix eigenvalues quickly

This trick only works for $2\times2$ matrices, but these are the most common in dynamical systems. In some extra code included above in the solutions above I used the following formula to calculate the eigenvalues:
```math
\lambda_i = \frac{\mathrm{Tr}[A] \pm \sqrt{\mathrm{Tr}[A]^2 - 4\mathrm{Det}[A]}}{2}.
```
But where does this come from? Is it always true?
Of course I didn't invent this, see for example [Steven Strogatz's Book: nonlinear dynamics and chaos](https://www.stevenstrogatz.com/books/nonlinear-dynamics-and-chaos-with-applications-to-physics-biology-chemistry-and-engineering) at page 132. Old-fashioned physical copy available at Sussex library.
...
Usually, we have to calculate eigenvalues using the characteristic equation $\mathrm{det}(A - \lambda\mathbb{I}) = 0$ where $\mathbb{I}$ is the identity matrix. Then the polynomial will look like
...
c & d-\lambda
\end{bmatrix}\right) = 0
\quad\Rightarrow\quad (a -\lambda)(d-\lambda) - bc = 0.
```

Now recall that 
```math
\mathrm{Tr}\left[A\right] = a+d\quad\text{and}\quad\mathrm{Det}\left[A\right] = ad -bc.
```

Now by expanding the characteristic equation we will notice that we can write the solution (quadratic equation solution) in terms of the trace and the determinant of the matrix:
...
This is quite handy on the computer! On paper, not always.
Of course you can always use the `LinearAlgebra.jl` package ([documentation here](https://docs.julialang.org/en/v1/stdlib/LinearAlgebra/)) and the `eigen` function to also get the eigenvectors.
"""
